...
const signToken = userID => {
    return JWT.sign({
        iss : "ThatSkiPatroller",
        sub : userID
    }, "ThatSkiPatroller", {expiresIn : "1h"})
};
...
userRouter.post('/login', passport.authenticate('local', {session : false}), (req, res) => {
    // isauthenticated returns true if authenticated, false if not
    if(req.isAuthenticated()){
        const {_id, username, role} = req.user;
        const token = signToken(_id);
        // http only makes it so on client side cannot touch this cookie to prevent against cross site scripting attacks; 
        // sameSite prevents against cross site request forgery attacks
